...
[PROJECT_RULES.md](../foundation/PROJECT_RULES.md) lists the CI provider as "Not Yet Decided," noting GitHub Actions as the reasonable default given the monorepo ([ADR-004](0004-repository-structure.md)) and Vercel direction ([ADR-005](0005-deployment-target.md), proposed). ADR-004 already commits to CI running against the whole repository with Turborepo affected-package detection — it just doesn't say *where* CI runs. The repository currently has no remote; pushing to GitHub is a prerequisite for both the PR workflow in PROJECT_RULES and any hosted CI.
